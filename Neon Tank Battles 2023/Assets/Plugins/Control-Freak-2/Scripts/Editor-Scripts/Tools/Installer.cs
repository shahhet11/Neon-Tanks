...
public class Installer : EditorWindow
	...
	const string 
		...
		//EVP_ADD_ON_PATH 				= "Assets/Plugins/Control-Freak-2/Add-Ons/CF2-EVP.unitypackage",
	...
	static private void CheckInstallation()
		...
		if (prefs.isIsntalled && (prefs.projectPath != curProjPath))
			{
			EditorUtility.DisplayDialog(DIALOG_TITLE, "Looks like CF2 was installed in different location...", "OK");
	...
	void OnGUI()
		...
		CFGUI.BeginIndentedVertical(CFEditorStyles.Inst.transpSunkenBG);
			...
			DrawFeatureBox(this.inputAxesPresent, "Input Axes Present!", "Input Axes are missing or are incomplete!");
			DrawFeatureBox(this.defineSymbolsPresent, "Scripting Symbols Defined!", "Scripting Symbols are not defined for all platforms!");
	...
	private void ShowAddOnMenu()
		...
		AddMenuItem(menu, new GUIContent("Install \'Opsive Third Person Controller\' Add-On..."), this.InstallOpsiveTpcAddOn, this.opsiveTpcPresent);
	...
	//private void InstallEvpAddOn()				{ 	AssetDatabase.ImportPackage(EVP_ADD_ON_PATH, DONT_SHOW_ADD_ON_PACKAGE_IMPORT_WINDOW); }
	private void InstallOpsiveTpcAddOn()		{ 	AssetDatabase.ImportPackage(OPSIVE_TPC_ADD_ON_PATH, DONT_SHOW_ADD_ON_PACKAGE_IMPORT_WINDOW); }
	...
	public class PlatformOptionList
		...
		public void Refresh()
			...
			for (int i = 0; i < platformIds.Length; ++i)	
				{
				if (platformIds[i] == BuildTargetGroup.Unknown)
					continue;
