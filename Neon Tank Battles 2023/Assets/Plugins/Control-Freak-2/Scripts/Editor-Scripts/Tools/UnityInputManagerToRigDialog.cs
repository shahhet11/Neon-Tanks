...
public class UnityInputManagerToRigDialog : EditorWindow 
	...
	private void AddAxesToRig()
		...
		for (int i = 0; i < this.axisList.Count; ++i)
			...
			for (int ai = 0; ai < axisDesc.inputAxes.Count; ++ai)	
				...
				switch (inputAxis.type)
					...
					case UnityInputManagerUtils.AxisType.KeyOrMouseButton :
						KeyCode 	
							positiveCode 	= InputRig.NameToKeyCode(inputAxis.positiveButton),
							positiveAltCode = InputRig.NameToKeyCode(inputAxis.altPositiveButton),
							negativeCode 	= InputRig.NameToKeyCode(inputAxis.negativeButton),
							negativeAltCode = InputRig.NameToKeyCode(inputAxis.altNegativeButton);
					...
					case UnityInputManagerUtils.AxisType.MouseMovement :
						...
						if ((inputAxis.axis == UnityInputManagerUtils.MOUSE_X_AXIS_ID) ||
							...
							mouseDeltaBinding.AddTarget().SetSingleAxis(axisDesc.name, false); //, axisDesc.inputAxis.invert);
						...
						// Scroll wheel...
							...
							scrollBinding.Clear();
							scrollBinding.AddTarget().SetSingleAxis(axisDesc.name, false);
			...
			else if ((axisDesc.targetAxisType == InputRig.AxisType.SignedAnalog) || (axisDesc.targetAxisType == InputRig.AxisType.Analog))
				{
				float 
					gravity = 0,
					sensitivity = 0;
