...
public class RigAxisNameDrawer
	...
	private void ShowContextMenu(string curName, InputRig rig)
		...
		if ((curName.Length > 0) && (axisConfig == null))
			...
			menu.AddItem(new GUIContent("Create \"" + curName + "\" Analog Axis"), 			false, this.OnMenuCreateAxis, new AxisCreationParams(rig, curName, InputRig.AxisType.Analog));
			menu.AddItem(new GUIContent("Create \"" + curName + "\" Analog (Signed) Axis"),	false, this.OnMenuCreateAxis, new AxisCreationParams(rig, curName, InputRig.AxisType.SignedAnalog));
