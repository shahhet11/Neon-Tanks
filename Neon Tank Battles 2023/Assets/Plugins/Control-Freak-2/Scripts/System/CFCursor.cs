...
namespace ControlFreak2
{
...
#endif 
			bool changed = (mLockState != value);	

			mLockState = value;
			
			if (!NeedToPreventCursorLocking())
				{
				InternalSetLockMode(value);
				}

			if (changed && (onLockStateChange != null))
				onLockStateChange();
			}
	...
	static public bool visible 
		...
		set
			...
			if (!NeedToPreventCursorLocking())
				InternalSetVisible(value); //UnityEngine.Cursor.visible = value;
			}

		get 
			{
			return (NeedToPreventCursorLocking() ? mVisible : (mVisible = InternalIsVisible())); //UnityEngine.Cursor.visible));
			}
		}
...
//! \cond
	...
	static public void InternalRefresh()
		...
		if (NeedToPreventCursorLocking())
			{
			InternalSetLockMode(CursorLockMode.None); //UnityEngine.Cursor.lockState	= CursorLockMode.None;
			InternalSetVisible(true); //UnityEngine.Cursor.visible		= true;
			}
		else
			{
			InternalSetLockMode(mLockState);	//UnityEngine.Cursor.lockState	= mLockState;
			InternalSetVisible(mVisible); //UnityEngine.Cursor.visible		= mVisible;
			}
		}
