...
//! \cond
	...
	protected void SetOriginPos(Vector3 pos, bool animate)
		...
		if (animate)
			...
			{	
			this.SetWorldPos(this.originPos); //this.transform.position = this.originPos;
	...
	protected void UpdateOriginAnimation()
		...
		if (this.originAnimOn)
			...
			if (this.originAnimElapsed >= (this.originSmoothTime * ORIGIN_ANIM_MAX_TIME))
				{
				this.originAnimOn = false;
				this.SetWorldPos(this.originPos);	
				}
			else
				{
				this.SetWorldPos(Vector3.Lerp(this.originStartPos, this.originPos, 
	...
	override public bool OnTouchStart(TouchObject touch, TouchControl sender, TouchStartType touchStartType)
		...
		this.touchStateWorld.OnTouchStart	(startPosWorld,		curPosWorld, 0, this.touchObj.IsMouse());
		this.touchStateScreen.OnTouchStart	(startPosScreen,	curPosScreen, 0, this.touchObj.IsMouse());
		this.touchStateOriented.OnTouchStart(startPosOriented,	curPosOriented, 0, this.touchObj.IsMouse());
