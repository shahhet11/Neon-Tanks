...
//! \cond
	...
	protected class FingerState
		...
		public bool OnTouchStart(TouchObject touchObj, float delay, TouchStartType touchStartType)
			...
			this.touchScreen.OnTouchStart(startPos, touchObj.screenPosCur, delay, this.touchObj.IsMouse());
			this.touchOriented.OnTouchStart(
				this.zone.ScreenToOrientedPos(startPos, touchObj.cam), 
				this.zone.ScreenToOrientedPos(touchObj.screenPosCur, touchObj.cam), delay, this.touchObj.IsMouse());
	...
	protected class MultiFingerTouch 
		...
		public void Start(FingerState[] fingers, Vector2 startScreenPos, Vector2 startOrientedPos, bool quietMode, float quietAlreadyElapsed)
			...
			if (!quietMode)
				{
				this.touchScreen.OnTouchStart(startScreenPos, startScreenPos, 0, this.controlledByMouse);
				this.touchOriented.OnTouchStart(startOrientedPos, startOrientedPos, 0, this.controlledByMouse);
				}
			}
		...
		public void StartOfficially()
			...
			this.quietMode = false;
			this.touchScreen.OnTouchStart(this.posStartScreen, this.posCurScreen, this.quietModeElapsed, this.controlledByMouse);
			this.touchOriented.OnTouchStart(this.posStartOriented, this.posCurOriented, this.quietModeElapsed, this.controlledByMouse);
			}
		...
		private void StartDrivingControl(TouchControl c)
			...
			this.drivingTouch.Start(this.touchScreen.GetStartPos(), this.touchScreen.GetCurPosRaw(), this.zone.GetCamera(), false);
...
//Debug.Log("[" + Time.frameCount + "] Emulated finger OnTouchStart() LOOP detected!! (Zone: " + this.zone.name + ")"); 
		...
		public void StartTouches(int num, Vector2 center, bool isMouse)
			...
			for (int i = 0; i < num; ++i)
				...
				finger.Start(finger.emuPos, finger.emuPos, this.zone.GetCamera(), isMouse);
